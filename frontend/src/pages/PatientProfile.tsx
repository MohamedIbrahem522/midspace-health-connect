...
import {
  ...
  Clock, User, MapPin, Phone, Mail, X, Save,
} from "lucide-react";
...
export default function PatientProfile() {
    ...
    try {
      ...
      if (data.id) {
        ...
        const dobDisplay = dobRaw
          ...
          : "Not set";
        const dobInput = dobRaw ? dobRaw.split("T")[0] : "";
        setProfile(prev => ({
          ...prev,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editForm.bloodType) body.bloodType = editForm.bloodType;
      ...
      setProfile(prev => ({
        ...
        bloodType: editForm.bloodType || prev.bloodType,
        dateOfBirth: editForm.dateOfBirth
          ? new Date(editForm.dateOfBirth).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })
          : prev.dateOfBirth,
        dateOfBirthDisplay: editForm.dateOfBirth
          ? new Date(editForm.dateOfBirth).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })
          : prev.dateOfBirthDisplay,
      }));
      setEditing(false);
        ...
        <div className="container mx-auto max-w-5xl px-4 h-14 flex items-center justify-between">
          ...
          {editing ? (
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="h-8 text-xs gap-1.5" onClick={() => { setEditing(false); setEditForm({ ...profile }); }}>
                ...
                {editing ? (
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} className="mt-1 h-10 text-sm" />
                    </div>
                    <div>
                      <Label className="text-xs text-muted-foreground">Phone</Label>
                      <Input value={editForm.phone} onChange={(e) => setEditForm({ ...editForm, phone: e.target.value })} className="mt-1 h-10 text-sm" />
                    </div>
                    <div>
                      <Label className="text-xs text-muted-foreground">Date of Birth</Label>
                      <Input value={editForm.dateOfBirth} onChange={(e) => setEditForm({ ...editForm, dateOfBirth: e.target.value })} className="mt-1 h-10 text-sm" placeholder="YYYY-MM-DD" />
                    </div>
                    <div>
                      <Label className="text-xs text-muted-foreground">Blood Type</Label>
                      <Input value={editForm.bloodType} onChange={(e) => setEditForm({ ...editForm, bloodType: e.target.value })} className="mt-1 h-10 text-sm" />
                    </div>
                  </div>
                ) : (
                  <div className="space-y-3">
